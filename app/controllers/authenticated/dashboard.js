...
export default Controller.extend({
  ...
  // Returns true if NowWhat is happening and user has not accepted the challenge
  displayNowWhatBanner: computed('currentUser.user.projects.[]', 'primaryProject.currentProjectChallenge.count', function () {
    let ps = this.get('currentUser.user.projects');
    ...
    // Set a local variable for the correct challenge
    let newc = null;
    // Loop through the challenges to find the latest NowWhat event
    cs.forEach(function (c) {
      // If this is an event
      if (c.eventType == 3) {
        // If the challenge is newer than ones already found
